...
var EventSender = Ember.Object.extend(Ember.Evented);
...
export default Ember.Component.extend({
    ...
    if (dropdown) {
      var ownActions = {
        ...
        scrollTo: function scrollTo(option, e) {
          return _this3.send('scrollTo', option, dropdown, e);
        }
      };
      return {
  ...
  actions: {
    ...
    scrollTo: function scrollTo(option /*, dropdown, e */) {
      ...
      var index = this.indexOfOption(option);
      if (index === -1) {
        return;
      }
  ...
  advanceSelectableOption: function advanceSelectableOption(activeHighlighted, step) {
    ...
    var _optionAtIndex2 = this.optionAtIndex(startIndex);

    var disabled = _optionAtIndex2.disabled;
    var option = _optionAtIndex2.option;

    while (option && disabled) {
      var next = this.optionAtIndex(startIndex += step);
  ...
  _handleTriggerTyping: function _handleTriggerTyping(dropdown, e) {
    ...
    var matches = this.filter(this.get('results'), term, true);
    if (get(matches, 'length') === 0) {
      return;
    }
    var firstMatch = _optionAtIndex(matches, 0);
